...
class ConceptDiscovery(object):
  ...
  """
    ...
    """
        ...
        self.discovery_images_index = None
        cifar10_classes = ['plane', 'car', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck']

        self.model = model
        self.sess = sess
        self.target_class = target_class
        self.class_number = cifar10_classes.index(target_class)
    ...
    def load_images_target_in_50k(self, class_number):
        ...
        img_dir = "E:/project/lantentspace/static/data/CIFAR10/pic/random_50k_png"
        # Step 2: 找到标签为class_number的所有图片的索引
        target_indices = [idx for idx, label in enumerate(data_z_labels) if label == class_number]

        # Step 3: 构建文件名列表
        image_filenames = ["pic_{}.png".format(idx) for idx in target_indices]
        ...
        for filename in image_filenames:
            ...
            if os.path.exists(image_path):
                ...
                image = load_image_from_file(image_path, self.image_shape)

                images.append(image)
                # if len(images) >= 100:
                #     break

        return np.array(images), target_indices
    ...
    """
        if param_dict is None:
            param_dict = {}
        dataset, image_numbers, patches, image_number_single_class = [], [], [], []

        # 加载50k中target_class的图片
        if discovery_images is None:
            # raw_imgs = self.load_concept_imgs(self.target_class, self.num_discovery_imgs)
            raw_imgs, raw_image_index = self.load_images_target_in_50k(self.class_number)
            self.discovery_images = raw_imgs
            self.discovery_images_index = raw_image_index
            print("该类别图片共：", raw_imgs.shape)
        else:
        ...
        if self.num_workers:
            ...
            for fn, img in enumerate(self.discovery_images):
                ...
                for superpixel, patch in zip(image_superpixels, image_patches):
                    dataset.append(superpixel)
                    patches.append(patch)
                    image_numbers.append(self.discovery_images_index[fn])
                    image_number_single_class.append((fn))
        self.dataset, self.image_numbers, self.patches, self.image_number_single_class = \
            np.array(dataset), np.array(image_numbers), np.array(patches), np.array(image_number_single_class)
    ...
    """
        # 1. segmentation.slic
        # SLIC (Simple Linear Iterative Clustering) 是一种超像素分割算法，它将图像分割成具有相似颜色和纹理的小块。
        #
        # 参数：
        #
        # img：输入图像。
        # n_segments：生成的超像素数，默认为 100。
        # compactness：空间距离与颜色距离的权衡因子，默认为 10。较高的值会生成更紧凑的超像素。
        # sigma：应用于每个通道的高斯平滑的标准偏差，默认为 0。
        # 2. segmentation.watershed
        # 分水岭算法是基于梯度的图像分割方法，常用于标记分割。
        #
        # 参数：
        #
        # img：输入图像。
        # markers：标记点的数组，指定分割开始的位置。
        # compactness：控制区域形状的参数。较高的值会生成更紧凑的分割区域。
        # 3. segmentation.quickshift
        # Quickshift 是一种基于核的快速图像分割算法。
        #
        # 参数：
        #
        # img：输入图像。
        # kernel_size：用于近似密度梯度的核大小，默认为 5。较大值会生成较少、较大的分割。
        # max_dist：控制链接距离的最大距离，默认为 10。较大值会生成更多的分割。
        # ratio：平滑项与颜色项的权衡因子，默认为 1.0。较大值会生成更平滑的分割。
        # 4. segmentation.felzenszwalb
        # Felzenszwalb 的高效图像分割算法。
        #
        # 参数：
        #
        # img：输入图像。
        # scale：控制分割块大小的尺度参数，默认为 1。较高的值会生成较大的分割区域。
        # sigma：用于预处理的高斯平滑标准偏差，默认为 0.8。
        # min_size：每个分割区域的最小大小，默认为 20。较高的值会生成较少的分割。
        if param_dict is None:
            param_dict = {}
        if method == 'slic':
            n_segmentss = param_dict.pop('n_segments', [4, 8])
            n_params = len(n_segmentss)
            compactnesses = param_dict.pop('compactness', [40] * n_params)
    ...
    """
        ...
        for bn in self.bottlenecks:
            ...
            if activations is None or bn not in activations.keys():
                print('----------------------------------')
                print('dataset.shape: 一共发现的超像素个数', self.dataset.shape)
                bn_activations = self._patch_activations(self.dataset, bn)
                print('bn_activations: avgpool层输出形状', bn_activations.shape)
            ...
            for i in range(bn_dic['label'].max() + 1):
                # 当前label的索引,一个类别5000张图，划分完以后大概50000张概念图，聚10个类别以后，设置聚类阈值最小img=5000
                ...
                if len(label_idxs) > self.min_imgs:
                    ...
                    # 每个概念对应的图片number,看看这个概念在多少张不同的图片中出现
                    ...
                    if highly_common_concept or cond2 or cond3:
                        ...
                        bn_dic[concept] = {
                            ...
                            'image_number_single_class': self.image_number_single_class[concept_idxs]
                        }
                        bn_dic[concept + '_center'] = centers[i]
                        print("当前概念：", concept)
                        print("当前概念相关超像素个数：", concept_idxs.shape)
    ...
    """
        if directory is None:
            directory = self.cav_dir
        # params = tf.HParams(model_type='linear', alpha=.01)
        Params = namedtuple('Params', ['model_type', 'alpha'])

        params = Params(model_type='linear', alpha=0.01)
