...
from models.BigGAN_CIFAR10 import BigGAN_CIFAR10
...
class ConditionedBigGAN(nn.Module):
    ...
    # 从targetclass中挑选batchsize个，也就是batch中的每一个对应一个类别
...
class CIFAR10BigGAN(nn.Module):
    def __init__(self, big_gan, target_classes=(1)):
        super(CIFAR10BigGAN, self).__init__()
        self.big_gan = big_gan
        ...
        self.dim_shift = 80
        # 加载保存的后128维度的向量
        self.tail_128_elements = torch.load('E:/project/lantentspace/GANLatentDiscovery-master/models/BigGAN_CIFAR10'
                                            '/label_z_128.pt')
    ...
    def forward(self, z, classes=None):
        ...
        # 需要填充后128位
        # 获取该batch对应类别的后128维
        tail_128_batch = torch.stack([self.tail_128_elements[class_id] for class_id in classes]).to(z.device)

        # 将z和tail_128_batch拼接起来
        # print(z.device, tail_128_batch.device)
        z_extended = torch.cat((z, tail_128_batch), dim=1)

        return self.big_gan(z_extended)
...
# 装饰器 @gan_with_shift 在代码中动态地为 ConditionedBigGAN 类添加了gan_with_shift中的方法
@gan_with_shift
...
@gan_with_shift
def make_big_gan_cifar10(G_weights, target_class):
    checkpoints_path = G_weights
    G = BigGAN_CIFAR10.Generator().to(device)
    G.load_state_dict(torch.load(checkpoints_path, map_location=device)["state_dict"])
    G.eval()
    return CIFAR10BigGAN(G, target_class)
