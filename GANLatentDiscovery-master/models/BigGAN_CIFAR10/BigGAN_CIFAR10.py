...
# 这个bigGAN只让GenBlock最后一个卷积层变成拆分卷积
...
class SeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, norm_layer=nn.InstanceNorm2d,
        ...
        self.conv = nn.Sequential(
            snconv2d(in_channels=in_channels, out_channels=in_channels * scale_factor, kernel_size=kernel_size,
                      stride=stride, padding=padding, groups=in_channels, bias=use_bias),
            norm_layer(in_channels * scale_factor),
            snconv2d(in_channels=in_channels * scale_factor, out_channels=out_channels,
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.bottom = 4
#        self.affine_list = []
        # self._100D_to_1D = _100D_to_1D()
        # self._100D_to_1D.load_state_dict(torch.load("./_100D_to_1D.pt"))

    def forward(self, z, label=None, shared_label=None, eval=False):
        affine_list = []
        
        if label is None and shared_label is None:
            # 这时候的z的输入就是 [batch_size, 208], 前面80就是z，后面128就是shared_label
            z0 = torch.split(z, 80, 1)
            pure_z = z0[0]
            zs = torch.split(pure_z, 20, 1)
            z = zs[0]
            shared_label = torch.cat((z0[1], z0[2]), dim=1)
        else:
            z0 = z
        ...
        shared_label = shared_label.reshape(shared_label.shape[0], -1)
