...
def make_interpolation_chart(G, deformator=None, z=None,
    ...
    # rows_count = (dims_count + cols - 1) // cols
    # fig, axs = plt.subplots(rows_count, cols, figsize=(50, 3 * rows_count), **kwargs)
    #
    # for ax in axs.flat:
    #     ax.axis('off')
    #
    # axs[0, 0].imshow(to_image(original_img, True))
    # if texts is None:
    #     texts = dims
    #
    # for idx, (dim, text) in enumerate(zip(dims, texts)):
    #     row = (idx // cols)
    #     col = (idx % cols)
    #     shifts_imgs = interpolate(G, z, shifts_r, shifts_count, dim, deformator)
    #     axs[row, col].imshow(to_image(make_grid(shifts_imgs, nrow=(2 * shifts_count + 1), padding=1), True))
    #     axs[row, col].text(-20, 21, str(text), fontsize=10)
    #
    # if deformator is not None and deformator_is_training:
    #     deformator.train()
    #
...
@torch.no_grad()
def inspect_all_directions(G, deformator, out_dir, zs=None, num_z=4, shifts_r=8.0):
