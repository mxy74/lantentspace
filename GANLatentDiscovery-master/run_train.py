...
def main():
    ...
    # model-specific 239伯恩山，
    parser.add_argument('--target_class', nargs='+', type=int, default=[1],
    ...
    # 根据命令行参数设置CUDA设备和随机种子，以确保实验的可重复性：
    ...
    # 初始化潜在空间的变形器模型，那个A矩阵
    # shift_dim：G的z的维度  ，input_dim：希望变形的方向，out_dim，与潜空间维度相同，type：变形器的类型 default='ortho'
    ...
    # 那个偏移预测器reconstructor default='ResNet'
...
def save_results_charts(G, deformator, params, out_dir):
    deformator.eval()
    G.eval()
    z = make_noise(4, G.dim_z, params.truncation).cuda()
